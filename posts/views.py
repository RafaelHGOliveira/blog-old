...
class PostDetalhes(View):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.template_name = 'posts/post_detalhes.html'
        pk = self.kwargs.get('pk')
        post = get_object_or_404(Post, pk=pk, publicado_post=True)
        self.contexto = {
            'post': post,
            'comentarios': Comentarios.objects.filter(post_comentario=post
...
#         context["comentarios"] = Comentarios.objects.filter(publicado_comentario=True
...
#         comentario.save()
#         messages.success(self.request, 'Coment[ário enviado.')
